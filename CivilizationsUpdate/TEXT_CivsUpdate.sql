...
'Spend Builder charges to complete 25% of the original district cost.'),
('LOC_BUILDING_TLACHTLI_DESCRIPTION_ZJ',
'A building unique to the Aztecs. Provides 2 [ICON_Amenities] Amenities, +2 [ICON_Culture] Culture, +2 [ICON_Faith] Faith, +2 [ICON_Gold] Gold (duplicate yield each era since constructed or last repaired.), and a [ICON_GREATGENERAL] Great General Point. Provides +1 [ICON_Tourism] Tourism after advancing to the Conservation Civic.'),
...
'+50% [ICON_Production] Production towards wonders if placed next to a River. Wonders build at Ancient era give one [ICON_TradeRoute] trade route capacity and a free trader. Does not receive damage from Floods.'),
...
'+50% [ICON_Production] Production toward Theater Square and its buildings. +20% [ICON_Production] Production toward Medieval, Renaissance, and Industrial era wonders. [ICON_Tourism] Tourism from wonders of any era is +100%.'),
...
('LOC_TRAIT_LEADER_MAGNIFICENCES_DESCRIPTION_ZJ', --Magnificence
'Improved Luxury resources adjacent to Theater Square districts or Châteaux receive +2 [ICON_Culture] Culture. May initiate the Court Festival project in any city with a Theater Square district. Unlock Drama and Poetry Civic when Foreign Trade is discovered.'),
